...
std::size_t BigInt::reserve_size = 10;
...
BigInt::BigInt(const BigInt& copyed):memory(new char[copyed.maxsize]),is_positive(copyed.is_positive),maxsize(copyed.maxsize),size(copyed.size){
	std::copy(copyed.memory, copyed.memory + copyed.size, memory);
}
...
bool BigInt::operator==(const BigInt& a) const{
	...
	for (std::size_t i = 0; i < size; i++) if (memory[i] != a.memory[i]) return false;
	return true;
}
...
BigInt operator+(const BigInt &b, const BigInt& a)
{
	...
	if ((b.is_positive && a.is_positive) || (!b.is_positive && !a.is_positive)) {
		...
		if (add && std::max(b.size, a.size) == b.maxsize) {// Нужно выделить больше памяти
			...
			if (add) ans.memory[std::max(b.size, a.size)] = 1;
			ans.size = (add) ? std::max(b.size, a.size) + 1 : std::max(b.size, a.size);
			ans.maxsize = std::max(b.maxsize, a.maxsize);
		}
	}
	else {
		...
		for (std::size_t i = less->size; i < greater->size; i++) {
